...
class _ExploreEventState extends State<ExploreEvent> {
  ...
  Future<void> _loadContent(bool isFirst, String? term) async {
    ...
    final responses = await ApiService.get(endpointEvent);

    if (!mounted) return;
    
    final resultEvent = responses;

    await _precacheAllImages(context, resultEvent!['data']);
  ...
  Future<void> _precacheAllImages(BuildContext context, List<dynamic> events) async {
